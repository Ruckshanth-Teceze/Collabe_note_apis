...
import { createNoteSchema, shareNoteSchema, updateNoteSchema, } from "../dto/note.dto.js";
export class NoteController {
    ...
    async shareNote(c) {
        try {
            ...
            const result = await noteServices.shareCreate(validateShareNote.noteId ?? "", validateShareNote.userIds ?? []);
            return c.json({ success: true, result }, 201);
        }
        catch (error) {
            if (error instanceof ZodError) {
