...
export class FileService {
    ...
    async getPresignedUrl(userId, attachmentId) {
        // Get attachment
        ...
        const userPermission = await userRepository.getPermission(attachment.noteId, userId);
        if (!userPermission) {
            throw Error("Access denied - you don't have permission to access this file");
        }
    ...
    async getFilesByNoteId(userId, noteId) {
        ...
        const filesWithUrls = await Promise.all(attachments.map(async (attachment) => {
            const command = new GetObjectCommand({
                Bucket: BUCKET,
                Key: attachment.storageKey,
            });
    ...
    async deleteFile(userId, attachmentId) {
        // Get attachment
        const attachment = await fileRepository.getAttachmentById(attachmentId);
        if (!attachment) {
            throw Error("File not found");
        ...
        const userPermission = await userRepository.getPermission(attachment.noteId, userId);
        if (!userPermission) {
            throw Error("Access denied - you don't have permission to delete this file");
        }
        // Only OWNER and EDITOR can delete files
        if (!["OWNER", "EDITOR"].includes(userPermission.role)) {
            throw Error("Insufficient permissions - only owners and editors can delete files");
        }
        // Delete from MinIO
