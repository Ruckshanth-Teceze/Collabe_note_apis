...
export class FileController {
  async createFile(c: Context) {
    try {
      ...
      if (!uuidRegex.test(noteId)) {
        return c.json(
          { success: false, message: "Invalid noteId format" },
          400,
        );
      }
      ...
      const result = await fileService.attachmentCreate(
        userId || "",
        noteId,
        file,
      );
  ...
  async getFile(c: Context) {
    try {
      ...
      if (!attachmentId) {
        return c.json(
          { success: false, message: "attachmentId is required" },
          400,
        );
      }
  ...
  async getFilesByNoteId(c: Context) {
    try {
      const userId = await c.get("userId");
